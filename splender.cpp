...
bool check_inboard(Pos t) {
  // TODO: Task 1-1
  if(t.x>=0 && t.x<BOARD_HEIGHT && t.y>=0 && t.y<BOARD_WIDTH){
    return 1;
  }
  return 0;
}
...
bool check_swap(Pos a, Pos b) {
  // TODO: Task 1-3
  if(check_inboard(a) && check_inboard(b) && (abs((a.x-b.x)+(a.y-b.y)))==1){
    if(gameboard[a.x][a.y].ability==ABI_KILLSAME || gameboard[a.x][a.y].ability==ABI_BOMB || gameboard[b.x][b.y].ability==ABI_KILLSAME || gameboard[b.x][b.y].ability==ABI_BOMB){
      if(gameboard[a.x][a.y].ability==ABI_KILLSAME && gameboard[b.x][b.y].ability==ABI_BOMB){
        return 0;
      }
      if(gameboard[a.x][a.y].ability==ABI_BOMB && gameboard[b.x][b.y].ability==ABI_KILLSAME){
        return 0; 
      }
      else{
        return 1;
    ...
    if(check_eliminate(nullptr)){
      gameboard[b.x][b.y]=gameboard[a.x][a.y];
      gameboard[a.x][a.y]=tmp;
      return 1;
    }
    gameboard[b.x][b.y]=gameboard[a.x][a.y];
    gameboard[a.x][a.y]=tmp;
...
void apply_bomb(Pos pos) {
  ...
  for(int i=max((pos.x-2), 0);i<=min((pos.x+2), (BOARD_HEIGHT-1));++i){
    for(int j=max((pos.y-2), 0);j<=min((pos.y+2), (BOARD_WIDTH-1));++j){
      if(gameboard[i][j].ability>ABI_NORMAL && elimi_tags[i][j]==0){
        Pos tmp={i, j};
        apply_special(tmp, pos);
      }
      elimi_tags[i][j]=1;
    }
  }
...
void apply_killsame(Pos pos, Pos tar) {
  // TODO: Task 2-2
  int tmp_type=gen_rand_type();
  elimi_tags[pos.x][pos.y]=1;
  if(gameboard[tar.x][tar.y].type==GEM_NULL || gameboard[tar.x][tar.y].ability==ABI_KILLSAME){
    for(int i=0;i<BOARD_HEIGHT;++i){
      for(int j=0;j<BOARD_WIDTH;++j){
        if(gameboard[i][j].type==tmp_type){
          if(gameboard[i][j].ability>ABI_NORMAL && elimi_tags[i][j]==0){
            Pos tmp={i, j};
            apply_special(tmp, pos);
            elimi_tags[i][j]=1;
    ...
    for(int i=0;i<BOARD_HEIGHT;++i){
      for(int j=0;j<BOARD_WIDTH;++j){
        if(gameboard[i][j].type==gameboard[tar.x][tar.y].type && gameboard[tar.x][tar.y].type!=GEM_NULL){
          if(gameboard[i][j].ability==ABI_NORMAL){
            elimi_tags[i][j]=1;
          }
          else if(gameboard[i][j].ability>ABI_NORMAL && elimi_tags[i][j]==0){
            Pos tmp={i, j};
            apply_special(tmp, pos);
            elimi_tags[i][j]=1;
          }
...
void apply_cross(Pos pos) {
  ...
  elimi_tags[pos.x][pos.y]=1;
  for(int i=0;i<BOARD_HEIGHT;++i){
    if(elimi_tags[i][pos.y]==0 && gameboard[i][pos.y].ability>1){
      Pos tmp={i, pos.y};
      apply_special(tmp, pos);
      elimi_tags[i][pos.y]=1;
    }
    elimi_tags[i][pos.y]=1;
  }
  for(int i=0;i<BOARD_WIDTH;++i){
    if(elimi_tags[pos.x][i]==0 && gameboard[pos.x][i].ability>1){
      Pos tmp={pos.x, i};
      apply_special(tmp, pos);
      elimi_tags[pos.x][i]=1;
    }
    elimi_tags[pos.x][i]=1;
  }
}
